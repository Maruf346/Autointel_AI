...
class ChattRequest(BaseModel):
    ...
    history: Optional[List[ChattMessage]] = Field(
        default_factory=list,
        description=(
            "Optional prior conversation turns for multi-turn context. "
            "Pass [] or omit for a fresh single-turn query."
        )
    )
...
async def chatt(request: ChattRequest):
    ...
    **Stateless automotive chat** - a lightweight, session-free chat endpoint
    ...
    - **Multi-turn support via `history`:** Pass previous `user`/`assistant` turns
      in the `history` array to maintain conversational context across requests.
      The caller is responsible for storing and forwarding history.
    - Strictly scoped to vehicle diagnostics and automotive troubleshooting.
    - Refuses any questions outside automotive scope.
    ...
    **Multi-turn request with history:**
    ```json
    {
      "message": "Could it be the ignition coil?",
      "history": [
        {"role": "user",      "content": "What does P0300 mean?"},
        {"role": "assistant", "content": "P0300 indicates a random/multiple cylinder misfire..."}
      ]
    }
    ```
    """
