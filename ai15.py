"""
Autointel Diagnostics - Tiered Backend v6.1
============================================
...
Endpoints:
  ...
  POST /api/chat                   ← Follow-up chat (backend calls only for paid users)
...
# ============================================================================
# CHAT  (backend calls only for paid users)
# ============================================================================
...
@app.post(
    ...
    tags=["Chat"],
    summary="Follow-up chat — backend calls after verifying payment"
)
async def chat(request: ChatRequest):
    """
    **Follow-up chat** with AI about an existing diagnosis.
    ...
    - Refuses any questions outside automotive scope (internal systems, unrelated topics, etc.).
    - The calling backend only invokes this after verifying payment.
    """
    session = _get_session(request.session_id)
    v = session['vehicle']

    # Vehicle + diagnosis context injected as the first user turn so the
    # model has the session data while the CHAT_SYSTEM_PROMPT governs behaviour.
    vehicle_context = (
    ...
    # Build message list:
    #   [system]  → strict automotive-only prompt (CHAT_SYSTEM_PROMPT)
    #   [user]    → vehicle/diagnosis context (first turn, injected once)
    ...
    # Persist only real conversation turns (not the injected context)
